...
-   **Agent (BOSSS XDR Agent)**: Software installed on monitored endpoints (servers, workstations, cloud instances) that collects security data (logs, file integrity, configuration assessments, etc.) and forwards it to the BOSSS XDR Manager.
-   **Alert**: A notification generated by the BOSSS XDR Manager when an event or a series of events matches a predefined rule, indicating a potential security issue, misconfiguration, or policy violation.
-   **API (BOSSS XDR API)**: An application programming interface provided by the BOSSS XDR Manager that allows for programmatic interaction with the Wazuh system, such as managing agents, retrieving alerts, updating rulesets, and checking system health.
...
-   **Cluster**:
    -   **BOSSS XDR Indexer Cluster (OpenSearch/Elasticsearch Cluster)**: A group of interconnected BOSSS XDR Indexer nodes that work together to store, index, and search data, providing scalability and high availability.
    -   **BOSSS XDR Manager Cluster**: A group of Wazuh managers working together to provide load balancing and high availability for agent connections and event processing.
...
-   **Dashboard (BOSSS XDR Dashboard / OpenSearch Dashboards / Kibana)**: A web-based visualization tool used to explore, analyze, and visualize data stored in the BOSSS XDR Indexer. It provides dashboards, visualizations, and a query interface for security events and alerts. For Wazuh 5.0.0, this is typically OpenSearch Dashboards.
-   **Decoder**: A component in the BOSSS XDR Manager that parses and extracts relevant information (fields) from raw log messages or event data.
...
-   **Indexer (BOSSS XDR Indexer / OpenSearch / Elasticsearch)**: The component responsible for storing, indexing, and making searchable the alerts and event data generated by the BOSSS XDR Manager. For Wazuh 5.0.0, this is typically OpenSearch.

**L**

-   **Log Analysis**: A core function of the BOSSS XDR Manager, involving the collection, normalization, parsing, and analysis of log data from various sources.

**M**

-   **Manager (BOSSS XDR Manager)**: The central component of the BOSSS XDR platform. It collects data from agents, analyzes it using rules and decoders, generates alerts, and manages agents.

**N**

-   **Node**:
    -   **BOSSS XDR Indexer Node**: A single instance of a BOSSS XDR Indexer (OpenSearch/Elasticsearch) process, typically running in a container. Multiple nodes can form a cluster.
    -   **BOSSS XDR Manager Node**: A single instance of a Wazuh manager, which can operate standalone or as part of a manager cluster.

**O**

-   **`ossec.conf`**: The main configuration file for the BOSSS XDR Manager and BOSSS XDR Agent.

**R**

-   **Rule**: A set of conditions defined in the BOSSS XDR Manager that, when met by an event or a sequence of events, trigger an alert.
-   **Ruleset**: The collection of all rules and decoders used by the BOSSS XDR Manager.
...
-   **BOSSS XDR API**: See API.
-   **BOSSS XDR Dashboard**: See Dashboard.
-   **BOSSS XDR Indexer**: See Indexer.
-   **BOSSS XDR Manager**: See Manager.
